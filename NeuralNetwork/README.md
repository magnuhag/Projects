...
The network is currently under development, and you (the reader) may wish to keep that in mind. Several problems are currently known:
  ...
  -An attempt will be made to implement automatic differentiation directly instead of indirectly; this is already done with Autograd, and is phenomenally inefficient. An attempt will be made to contruct Wengert lists and do reverse mode auto diff based on the network structure itself. Might be more efficient than calculating thousands of Jacobians explicitly.
