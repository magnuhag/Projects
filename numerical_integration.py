...
class Integrator:
    ...
    def func_eval(self):
        if self.a == float("-inf") and self.b == float("inf"):
            ...
            evaluated_function[1:-1] = self.f(t[1:-1]/(1-t[1:-1]**2))*(1+t[1:-1]**2)/(1-t[1:-1]**2)**2
            ...
            evaluated_function[1:-1] = self.f(self.a+t[1:-1]/(1-t[1:-1]))*1/(1-t[1:-1])**2
        elif self.a == float("-inf") and self.b != float("inf"):
            t = np.linspace(0, 1, self.n)
            evaluated_function = np.zeros(self.n)
            evaluated_function[0] = 0
            evaluated_function[-1] = self.f(self.b)
            evaluated_function[1:-1] = self.f(self.b-(1-t[1:-1])/t[1:-1])*1/t[1:-1]**2
        self.t = t
...
integrate = Integrator(f, float("-inf"), float("inf"), 100)
